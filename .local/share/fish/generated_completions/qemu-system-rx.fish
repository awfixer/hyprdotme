...
complete -c qemu-system-rx -o smp -d 'Simulate a SMP system with \\(aqn\\(aq CPUs initially present on the machine ty…'
...
complete -c qemu-system-rx -o global -d 'Set default value of driver\\(aqs property prop to value, e. g. : NDENT 7'
...
complete -c qemu-system-rx -o audio-help -d 'Will show the -audiodev equivalent of the currently specified (deprecated) en…'
complete -c qemu-system-rx -o audio -d 'This option is a shortcut for configuring both the guest audio hardware and t…'
...
complete -c qemu-system-rx -o no-acpi -d 'Disable ACPI (Advanced Configuration and Power Interface) support'
complete -c qemu-system-rx -o no-hpet -d 'Disable HPET support.  Deprecated, use \\(aq-machine hpet=off\\(aq instead'
...
complete -c qemu-system-rx -o qmp -d 'Like -monitor but opens in \\(aqcontrol\\(aq mode'
...
complete -c qemu-system-rx -o singlestep -d 'This is a deprecated synonym for the TCG accelerator property one-insn-per-tb'
complete -c qemu-system-rx -l preconfig -d 'Pause QEMU for interactive configuration before the machine is created, which…'
complete -c qemu-system-rx -s S -d 'Do not start CPU at startup (you must type \\(aqc\\(aq in the monitor)'
...
complete -c qemu-system-rx -s d -d 'Enable logging of specified items'
...
complete -c qemu-system-rx -o no-shutdown -d 'Don\\(aqt exit QEMU on guest shutdown, but instead only stop the emulation'
complete -c qemu-system-rx -o action -d 'The action parameter serves to modify QEMU\\(aqs default behavior when certain…'
...
complete -c qemu-system-rx -o nodefaults -d 'Don\\(aqt create default devices'
complete -c qemu-system-rx -o chroot -d 'Deprecated, use \\(aq-run-with chroot=. \\(aq instead'
...
complete -c qemu-system-rx -o semihosting -d 'Enable \\%Semihosting mode (ARM, M68K, Xtensa, MIPS, Nios II, RISC-V only)'
complete -c qemu-system-rx -o semihosting-config -d 'Enable and configure \\%Semihosting (ARM, M68K, Xtensa, MIPS, Nios II, RISC-V …'
...
complete -c qemu-system-rx -o async-teardown -d 'This option is deprecated and should no longer be used'
...
complete -c qemu-system-rx -o alt-grab -d 'and if you use -ctrl-grab then the modifier is the right Ctrl key (instead of…'
